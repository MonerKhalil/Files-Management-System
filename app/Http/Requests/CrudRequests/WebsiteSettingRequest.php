...
class WebsiteSettingRequest extends BaseRequest
{
    /**
    ...
    public function rules(): array
    {
        ...
        foreach ($dataKeys as $objectKey){
            $rules[$objectKey->key] = match ($objectKey->type){
                ...
                TypesFieldsEnum::FILE->value => $this->fileRule($objectKey->is_required),
                TypesFieldsEnum::IMAGE->value => $this->imageRule($objectKey->is_required),
                TypesFieldsEnum::EDITOR->value => $this->editorRule($objectKey->is_required),
    ...
    protected function mainData($keys){
        return app(WebsiteSettingRepository::class)->get(true,false,function ($q)use($keys){
